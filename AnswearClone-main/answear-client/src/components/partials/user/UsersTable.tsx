import { IconCircleDashedCheck, IconLock, IconLockOpen } from "@tabler/icons-react";
import { Button } from "components/ui";
import { IUser } from "interfaces/user";
import { useLockUserMutation, useUnlockUserMutation } from "services/user.ts";
...
const UsersTable: React.FC<UsersTableProps> = (props) => {
    ...
    const [unlockUser] = useUnlockUserMutation(); // Додано мутацію для розблокування
                    ...
                    {users?.map((user, index) => (
                        ...
                        >
                            ...
                            <td className="px-6 py-4 inline-flex text-right space-x-5">
                                ...
                                {isUserLockedOut(user.lockoutEnd) && (
                                    <Button onClick={() => unlockUser(user.id)} variant="iconTransparent" size="iconmd">
