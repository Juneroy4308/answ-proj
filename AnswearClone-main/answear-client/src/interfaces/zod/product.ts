...
const ProductVariationValueSchema = z.object({
    ...
    filters: z.preprocess(
        (value) => value ?? [],
        z
            .array(z.number({ message: "Фільтри повинні бути масивом позитивних чисел" }))
            //.nonempty({ message: "Фільтри є обов'язковими" }) // Власне повідомлення для required
            .refine((filters) => filters.every((filter) => filter > 0), {
