...
public class SlugService : ISlugService
{
    ...
    {
        var randomValue = Guid.NewGuid().ToString("N").Substring(0, 6);
        var transliteratedText = CyrillicToLatin($"{text}-{DateTimeOffset.UtcNow.ToUnixTimeMilliseconds()}-{randomValue}");
        return _slugHelper.GenerateSlug(transliteratedText);
    }
}
